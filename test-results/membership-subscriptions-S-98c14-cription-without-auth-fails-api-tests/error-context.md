...
- Name: membership\subscriptions.spec.ts >> Subscriptions >> create subscription without auth fails
- Location: tests\membership\subscriptions.spec.ts:71:7
...
> 79  |     expect(res.status()).toBe(401);
  ...
  133 |   });
  134 | 
  135 |   test('cleanup - delete plan', async ({ request }) => {
  136 |     if (!createdPlanId) return;
  137 |     const res = await request.delete(`${GATEWAY}/api/plans/${createdPlanId}`, {
