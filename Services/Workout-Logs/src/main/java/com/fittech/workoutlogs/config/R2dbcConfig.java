...
public class R2dbcConfig {
    ...
    public ConnectionFactory r2dbcConnectionFactory() {
        String connectionString = System.getenv("ConnectionStrings__workoutLogsDb");

        if (connectionString == null || connectionString.isBlank()) {
            throw new IllegalStateException(
                    "Missing environment variable: ConnectionStrings__workoutLogsDb");
        }
    ...
    private String require(Map<String, String> parts, String key) {
        ...
        if (value == null || value.isBlank()) {
            throw new IllegalStateException(
                    "Missing '" + key + "' in ConnectionStrings__workoutLogsDb");
        }
        return value;
    }
}
